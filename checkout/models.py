...
class Order(models.Model):
    ...
    home_Address_continued = models.CharField(
        max_length=60, null=False, blank=False
        )
    ...
    cost_of_order = models.DecimalField(
    ...
    grand_cost = models.DecimalField(
        max_digits=10, decimal_places=2,
        null=False, blank=False, default=0
        )
    original_pack = models.TextField(
        null=False, blank=False, default=''
        )
    stripe_p_id = models.CharField(
    ...
    def update_total(self):
        self.overall_cost = self.Lineitems.aggregate(
            Sum('lineitem_total'))['lineitem_total__sum'] or 0
        if self.overall_cost < settings.FREE_DELIVERY_LIMIT:
            self.cost_of_order = self.overall_cost * \
